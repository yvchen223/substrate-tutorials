...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn slash(
			...
			let (amount_to_slash, _) = T::Currency::slash(&target, amount);
			let (treasury_part, burned_part) = amount_to_slash.ration(1, 2);

			T::Currency::resolve_creating(&T::TreasuryAccount::get(), treasury_part);
			let burned = T::Currency::burn(burned_part.peek());
			burned
				.offset(burned_part)
				.try_drop()
